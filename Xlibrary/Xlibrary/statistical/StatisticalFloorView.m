...
@implementation StatisticalFloorView
{
NSInteger oldIndex_;
...
- (void)showAt:(NSInteger)index{
    if (oldIndex_ != index) {
        ...
        UIViewController *vc = [self.dataSource Statistical:self controllerAt:index];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(StatisticalFloorView:didSwitchTo:)]) {
            [self.delegate StatisticalFloorView:self didSwitchTo:index];
        }
    }
}
...
- (void)switchTo:(NSInteger)index{
    ...
    if (oldCtrl_ != nil && oldCtrl_.parentViewController == self.baseViewController) {
        ...
        UIViewController *newvc = [self.dataSource Statistical:self controllerAt:index];
...
- (void)repositionForOffsetX:(CGFloat)offsetx{
    ...
    if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInStatisticalView:self]) {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(StatisticalFloorView:switchingFrom:to:percent:)]) {
        [self.delegate StatisticalFloorView:self switchingFrom:oldIndex_ to:panToIndex_ percent:fabs(offsetx)/self.bounds.size.width];
    }
}
...
- (void)backToOldWithOffset:(CGFloat)offsetx{
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInStatisticalView:self] && panToIndex_ != oldIndex_) {
            //[self removeAt:panToIndex_];
            [oldCtrl_ beginAppearanceTransition:YES animated:NO];
            [self removeWill];
            [oldCtrl_ endAppearanceTransition];
        }
        if (self.delegate && [self.delegate respondsToSelector:@selector(StatisticalFloorView:switchCanceled:)]) {
            [self.delegate StatisticalFloorView:self switchCanceled:oldIndex_];
        }
    }];
...
- (void)panHandler:(UIPanGestureRecognizer *)pan{
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (panToIndex < 0 || panToIndex >= [self.dataSource numberOfControllersInStatisticalView:self]) {
            ...
            if (panToIndex != panToIndex_) {
                ...
                willCtrl_ = [self.dataSource Statistical:self controllerAt:panToIndex];
        ...
        if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInStatisticalView:self] && panToIndex_ != oldIndex_) {
            if (fabs(offsetx) > kPanSwitchOffsetThreshold) {
                ...
                [UIView animateWithDuration:animatedTime animations:^{
                    ...
                    if (self.delegate && [self.delegate respondsToSelector:@selector(StatisticalFloorView:didSwitchTo:)]) {
                        [self.delegate StatisticalFloorView:self didSwitchTo:oldIndex_];
                    }
                }];
            }
            else{
